...
void _main(void)
{
	...
	void* ptr_allocations[20] = {0};
	{
		...
		if ((freeFrames - sys_calculate_free_frames()) != 1 ) panic("Wrong allocation: either extra pages are allocated in memory or pages not allocated correctly on PageFile,%d",(freeFrames - sys_calculate_free_frames()));
		...
		if ((uint32) ptr_allocations[3] < (USER_HEAP_START + 4*Mega + 4*kilo) || (uint32) ptr_allocations[3] > (USER_HEAP_START + 4*Mega + 4*kilo + PAGE_SIZE)) panic("Wrong start address for the allocated space... %x\n",(USER_HEAP_START + 4*Mega + 4*kilo));
